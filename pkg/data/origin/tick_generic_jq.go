...
//nolint:funlen
func (g *TickGenericJQ) handle(ctx context.Context, pairs []Pair, body io.Reader) map[any]data.Point {
	points := make(map[any]data.Point)

	// Parse JSON data.
	var decoded any
	if err := json.NewDecoder(body).Decode(&decoded); err != nil {
		for _, pair := range pairs {
			points[pair] = data.Point{Error: err}
		}
		return points
	}
	...
	for _, pair := range pairs {
		...
		point := data.Point{Time: time.Now()}
		tick := Tick{Pair: pair}
