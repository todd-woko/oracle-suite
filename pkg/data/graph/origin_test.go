...
func TestOriginNode(t *testing.T) {
	tests := []struct {
		...
		{
			name:               "valid",
			dataPoint:          data.Point{Value: numericValue{x: bn.Float(1)}, Time: time.Now()},
			query:              "query",
		...
		{
			name:               "not fresh",
			dataPoint:          data.Point{Value: numericValue{x: bn.Float(1)}, Time: time.Now().Add(-30 * time.Second)},
			query:              "query",
		...
		{
			name:               "expired",
			dataPoint:          data.Point{Value: numericValue{x: bn.Float(1)}, Time: time.Now().Add(-60 * time.Second)},
			query:              "query",
			freshnessThreshold: time.Second * 20,
			expiryThreshold:    time.Second * 40,
			expectedFresh:      false,
